...
from .forms import BlogGeneratorForm, ReferenceBlogGeneratorForm
...
from .utils import (
    ...
    remove_plagiarism,
    gemini_paraphrase,
    generate_blog_content_with_reference,
...
def generate_blog(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            content = generate_blog_content(title, keywords, language)
...
@csrf_exempt
def reference_generate_blog(request):
    if request.method == "POST":
        form = ReferenceBlogGeneratorForm(request.POST)
        if form.is_valid():
            title = form.cleaned_data["title"]
            keywords = form.cleaned_data["keywords"]
            ...
            content = generate_blog_content_with_reference(title, keywords, reference, language, length)
            meta_description = generate_meta_description(title)
            image_url = generate_image(title)
        ...
        form = ReferenceBlogGeneratorForm()

    return render(request, "reference_generate_blog.html", {"form": form})
