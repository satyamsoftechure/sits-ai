...
def generate_blog_content(title, keywords, language):
    # Write a blog in hindi language for me reference site is this https://softechure.com/blog/important-factors-to-consider-when-building-a-website.
    prompt = f"Write a blog post in '{language}' language, titled '{title}'. Use the following keywords: {keywords}. The blog post should be informative, engaging, unique content, in proper HTML format with no css and between 1000-1500 words, generate_unique_content with your own logic to ensure the generated content is truly unique and plagiarism-free."
...
def generate_meta_description(title):
    prompt = f"Create a meta description of 170 words for the title '{title}' with no any heading only description ."
...
def generate_blog_content_with_reference(title, keywords, reference, language, length):
    prompt = f"Generate a blog post in '{language}' language, titled '{title}'. Use the following keywords: {keywords}. Ensure the content is original, plagiarism-free, and must be formatted correctly in proper HTML without CSS . The post should must be '{length}' words only and incorporate relevant information from these sources: '{reference}'."
